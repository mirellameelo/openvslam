...
# the input is Point 1 (X, Y) and Point 2 (X, Y)
...
load_counters = 0
#pontos vistos por determinado keyframe
...
if not counters_loaded:
    ...
    # keys = ('name', 'age', 'food')
    # values = ('Monty', 42, 'spam') -> dict = name: monty, age:42, food: spam
    # {'timestamp': [x, y, z]}
    ...
    #OUTPUT: (array([0, 1, 2, 2]), array([1, 0, 0, 2]))
        n_ground_points = np.count_nonzero(np.array(is_ground_point))
        print('n_ground_points: ', n_ground_points)

    #floor: arredondamento p baixo; 
    #pegando o que indica ser o menor valor p indicar o chao, isto eh, a referencia 
    ...
    print('grid_res: ', grid_res)
    #matriz de zeros = [linha, coluna] + tipo
...
#definindo as cores do mapa - linha
...
for x in xrange(grid_res[0]):
    for z in xrange(grid_res[1]):
        if visit_counter[x, z] == 0 or occupied_counter[x, z] == 0:
            ...
            grid_map[x, z] = 1 - occupied_counter[x, z] 
